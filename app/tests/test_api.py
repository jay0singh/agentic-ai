...
def test_chat_passes_session_through_and_shapes_response(client, monkeypatch):
    ...
    def fake_orchestrator(question, session_id=None, user_id=None):
        captured.update(question=question, session_id=session_id, user_id=user_id)
        return {
            "response": "an answer",
    ...
    r = client.post("/chat", json={"question": "What is X?", "session_id": "s-1"})
    assert r.status_code == 200
    body = r.json()
    assert body["answer"] == "an answer"
    assert body["steps_taken"] == ["vector_search", "generate", "judge"]
    assert captured == {"question": "What is X?", "session_id": "s-1", "user_id": None}
